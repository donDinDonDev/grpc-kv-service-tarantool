...
class TarantoolKeyValueStorageIntegrationTest {
  ...
  @Autowired private TarantoolSpaceInitializer initializer;
  ...
  void concurrentWritesFollowLastWriteWinsForSingleKey() throws Exception {
    try (TarantoolBoxClient firstClient = newTestClient();
      ...
      KeyValueStoragePort firstStorage =
          new TarantoolKeyValueStorage(
              firstClient,
              TEST_REQUEST_TIMEOUT,
              new TarantoolObservability(
                  new SimpleMeterRegistry(), ObservationRegistry.create(), Tracer.NOOP));
      KeyValueStoragePort secondStorage =
          new TarantoolKeyValueStorage(
              secondClient,
              TEST_REQUEST_TIMEOUT,
              new TarantoolObservability(
      ...
      try {
        ...
        Future<?> firstPut =
            executor.submit(() -> firstStorage.put(SHARED_KEY, StoredValue.bytes(new byte[] {1})));
        awaitContentionEntry(controlClient, 1L);
        assertThat(firstPut.isDone()).isFalse();

        Future<?> secondPut =
            executor.submit(() -> secondStorage.put(SHARED_KEY, StoredValue.bytes(new byte[] {2})));
  ...
  void readsRangeInBatchesWithExclusiveUpperBoundAndResumeCursor() {
    ...
    List<StoredEntry> firstBatch =
        this.storage.getRangeBatch(new RangeBatchQuery("b", "d", null, 1));
    List<StoredEntry> secondBatch =
        this.storage.getRangeBatch(new RangeBatchQuery("b", "d", firstBatch.getFirst().key(), 1));
    List<StoredEntry> thirdBatch =
        this.storage.getRangeBatch(new RangeBatchQuery("b", "d", secondBatch.getFirst().key(), 1));
  ...
  void readsRangeUsingUtf8LexicographicBounds() {
    ...
    List<StoredEntry> batch =
        this.storage.getRangeBatch(new RangeBatchQuery(utf8LowerKey, utf8UpperKey, null, 10));
  ...
  private void installContentionGate(TarantoolBoxClient tarantoolClient, String key)
      throws Exception {
    assertThat(booleanScalar(tarantoolClient, INSTALL_CONTENTION_GATE_SCRIPT, List.of(key)))
        .isTrue();
  }

  private void awaitContentionEntry(TarantoolBoxClient tarantoolClient, long expectedEntry)
      throws Exception {
    assertThat(numberScalar(tarantoolClient, WAIT_FOR_NEXT_CONTENTION_ENTRY_SCRIPT).longValue())
  ...
  private void executeBooleanScript(TarantoolBoxClient tarantoolClient, String script)
      throws Exception {
  ...
  private Boolean booleanScalar(String script) throws Exception {
    return this.client
        .eval(script, new TypeReference<List<Boolean>>() {})
        .get(5, TimeUnit.SECONDS)
        .get()
        .getFirst();
  }

  private Boolean booleanScalar(TarantoolBoxClient tarantoolClient, String script, List<?> args)
      throws Exception {
    return tarantoolClient
        .eval(script, args, new TypeReference<List<Boolean>>() {})
        .get(5, TimeUnit.SECONDS)
        .get()
        .getFirst();
  }
  ...
  private Number numberScalar(TarantoolBoxClient tarantoolClient, String script) throws Exception {
    return tarantoolClient
        .eval(script, new TypeReference<List<Number>>() {})
        .get(5, TimeUnit.SECONDS)
        .get()
        .getFirst();
  }
  ...
  private String stringScalar(String script) throws Exception {
    return this.client
        .eval(script, new TypeReference<List<String>>() {})
        .get(5, TimeUnit.SECONDS)
        .get()
        .getFirst();
  }
