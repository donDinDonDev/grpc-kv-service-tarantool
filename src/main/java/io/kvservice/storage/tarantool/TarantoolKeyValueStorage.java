...
public final class TarantoolKeyValueStorage implements KeyValueStoragePort {
  ...
  public TarantoolKeyValueStorage(
      TarantoolBoxClient client, Duration requestTimeout, TarantoolObservability observability) {
    this.client = client;
  ...
  public void put(String key, StoredValue value, Duration timeout) {
    this.observability.observe(
        "put",
        SafeLogFields.forPut(key, value),
        () -> {
    ...
    return this.observability.observe(
        ...
        () -> {
          ...
          SelectResponse<List<Tuple<List<?>>>> response =
              execute(
                  remainingTimeout -> {
                    SelectOptions remainingOptions =
                        SelectOptions.builder()
  ...
  private List<Object> tupleFor(String key, StoredValue value) {
    Object rawValue =
        value instanceof StoredValue.BytesValue bytesValue ? bytesValue.bytes() : null;
  ...
  private RangeScanStart resolveScanStart(RangeBatchQuery query) {
    ...
    if (startAfter == null
        || Utf8LexicographicKeyOrder.compare(startAfter, query.keyFromInclusive()) < 0) {
  ...
  private <T> T executeWithConflictRetry(
      Function<Duration, CompletableFuture<T>> requestFactory, Duration timeout) {
    return execute(requestFactory, timeout, true);
  }

  private <T> T execute(
      Function<Duration, CompletableFuture<T>> requestFactory,
      Duration timeout,
  ...
  private StorageAccessException mapDirectFailure(CompletionException exception) {
    ...
    if (cause instanceof CancellationException cancellationException) {
      return StorageAccessException.cancelled(
          "Tarantool request was cancelled", cancellationException);
    }
    return StorageAccessException.unavailable("Tarantool request failed", cause);
  }

  private boolean shouldRetryConflict(
      Throwable cause, boolean retryOnConflict, long deadlineNanos) {
