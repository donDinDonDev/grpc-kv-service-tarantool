...
public final class TarantoolObservability implements PoolEventListener {
  ...
  public TarantoolObservability(
      MeterRegistry meterRegistry, ObservationRegistry observationRegistry, Tracer tracer) {
  ...
  public <T> T observe(String operation, Map<String, String> safeFields, Supplier<T> action) {
    ...
    Observation observation =
        Observation.createNotStarted("kvservice.tarantool.operation", this.observationRegistry)
  ...
  public void onConnectionFailed(String host, int port, Throwable failure) {
    ...
    try (MdcScope ignored =
      ...
      LOGGER.warn(
          StructuredLogMessage.of(
              "Tarantool connection failed",
              connectionFields(
                  host,
                  port,
                  Map.of(
  ...
  public void onReconnectScheduled(String host, int port, long reconnectAfterMillis) {
    ...
    try (MdcScope ignored = MdcScope.open(connectionFields(host, port, fields))) {
      LOGGER.warn(
          StructuredLogMessage.of(
              "Tarantool reconnect scheduled", connectionFields(host, port, fields)));
    }
  }
  ...
  private void logFailure(
      String operation, String outcome, Map<String, String> safeFields, RuntimeException failure) {
    Map<String, String> fields = new LinkedHashMap<>();
  ...
  private Map<String, String> connectionFields(
      String host, int port, Map<String, String> extraFields) {
