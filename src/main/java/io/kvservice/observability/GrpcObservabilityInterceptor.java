...
public final class GrpcObservabilityInterceptor implements ServerInterceptor {
  ...
  private static final Propagator.Getter<Metadata> GRPC_METADATA_GETTER =
      new Propagator.Getter<>() {
        @Override
  ...
  public GrpcObservabilityInterceptor(
  ...
  public <ReqT, RespT> ServerCall.Listener<ReqT> interceptCall(
              ...
              withBaseFieldsAndTrace(
                  ...
                  () -> {
                    ...
                    recordRequestMetrics(
                        service, method, status, System.nanoTime() - startedAtNanos);
                    if (!status.isOk()) {
    ...
    Context context =
        Context.current().withValue(CorrelationIdSupport.REQUEST_ID_CONTEXT, requestId);
  ...
  private void recordRequestMetrics(
      String service, String method, Status status, long durationNanos) {
  ...
  private <T> T withBaseFieldsAndTrace(
      Map<String, String> baseFields, Span requestSpan, Supplier<T> action) {
  ...
  private void withBaseFieldsAndTrace(
      Map<String, String> baseFields, Span requestSpan, Runnable action) {
    try (MdcScope ignored = MdcScope.open(fieldsWithTrace(baseFields, requestSpan));
        TraceScope traceScope = TraceScope.open(this.tracer, requestSpan)) {
      action.run();
    }
