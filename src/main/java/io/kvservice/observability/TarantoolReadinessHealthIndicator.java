...
public final class TarantoolReadinessHealthIndicator implements HealthIndicator {
    ...
    public Health health() {
        try {
            List<Boolean> response = this.client.eval(READINESS_SCRIPT, new TypeReference<List<Boolean>>() {
            }).get(Math.max(1L, this.requestTimeout.toMillis()), TimeUnit.MILLISECONDS).get();
