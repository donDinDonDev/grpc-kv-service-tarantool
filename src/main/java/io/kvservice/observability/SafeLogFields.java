...
public final class SafeLogFields {
  ...
  public static Map<String, String> forRequest(Object request) {
    if (request instanceof PutRequest putRequest) {
      ...
      fields.put(
          "value.bytes",
          Integer.toString(
              value.getKindCase() == NullableBytes.KindCase.DATA ? value.getData().size() : 0));
      return Map.copyOf(fields);
    }
  ...
  public static Map<String, String> forPut(String key, StoredValue value) {
    ...
    fields.put(
        "value.bytes",
        Integer.toString(value.isNull() ? 0 : ((StoredValue.BytesValue) value).bytes().length));
    return Map.copyOf(fields);
  }
  ...
  public static Map<String, String> forKey(String fieldPrefix, String key) {
    ...
    fields.put(
        fieldPrefix + ".bytes", Integer.toString(key.getBytes(StandardCharsets.UTF_8).length));
