...
public final class KvGrpcService extends KvServiceGrpc.KvServiceImplBase {
    ...
    executeUnary(
        ...
        () ->
            GetResponse.newBuilder()
                .setRecord(
                    this.valueMapper.toRecord(
                        this.getValueUseCase.execute(
                            request.getKey(), this.requestBudgetFactory.create())))
                .build());
  }
  ...
  public void range(RangeRequest request, StreamObserver<RangeItem> responseObserver) {
    ...
    try {
      ...
      GrpcRangeResponseWriter responseWriter =
          new GrpcRangeResponseWriter(serverObserver(responseObserver), requestBudget);
      this.rangeValueUseCase.stream(
          ...
          entry -> {
            responseWriter.write(
                RangeItem.newBuilder().setRecord(this.valueMapper.toRecord(entry)).build());
  ...
  private ServerCallStreamObserver<RangeItem> serverObserver(
      StreamObserver<RangeItem> responseObserver) {
