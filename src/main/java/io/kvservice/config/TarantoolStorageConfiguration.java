...
public class TarantoolStorageConfiguration {
  ...
  TarantoolBoxClient tarantoolBoxClient(
      KvServiceProperties properties, TarantoolObservability observability) throws Exception {
  ...
  @Bean
  @ConditionalOnProperty(
      prefix = "kvservice.tarantool.init",
      name = "enabled",
      havingValue = "true",
      matchIfMissing = true)
  TarantoolSpaceInitializer tarantoolSpaceInitializer(
      TarantoolBoxClient client, KvServiceProperties properties) {
    KvServiceProperties.Tarantool tarantool = properties.getTarantool();
    TarantoolSpaceInitializer initializer =
        new TarantoolSpaceInitializer(
            client,
  ...
  KeyValueStoragePort keyValueStoragePort(
    ...
    return new TarantoolKeyValueStorage(
        client, properties.getTarantool().getRequestTimeout(), observability);
  }

  @Bean("tarantoolHealthIndicator")
  TarantoolReadinessHealthIndicator tarantoolHealthIndicator(
      TarantoolBoxClient client, KvServiceProperties properties) {
    return new TarantoolReadinessHealthIndicator(
        client, properties.getTarantool().getRequestTimeout());
  }
}
